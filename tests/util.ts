...
export class Util {
	...
	static async consoleLogCurrentUrl(page: Page, message?: string): Promise<void> {
		// eslint-disable-next-line @typescript-eslint/await-thenable
		const url = await page.url()
		const timestamp = new Date().toISOString().split('T')[1].slice(0, 12) // e.g., "14:23:05.123"
		const prefix = message ? `[${timestamp}] ${message}` : `[${timestamp}] Current URL`
