...
export class VirtualMachineComponent implements OnInit, DoCheck, OnDestroy {
		...
		startVM(flavor: string, servername: string, project: string, projectid: string | number): void {
				...
				if (this.selectedImage && flavor && servername && project) {
						...
						let play_information: any = this.getPlaybookInformation();
						if (play_information) {
								this.playbook_run = 1;
						} else {
		...
		getPlaybookInformation(): any {
				...
				if (
						...
						if (this.resEnvComponent.selectedTemplate.template_name) {
								playbook_info[this.resEnvComponent.selectedTemplate.template_name] = {
										...
										needs_forc_support: this.resEnvComponent.selectedTemplate.needs_forc_support}
								};
								playbook_info['user_key_url'] = {user_key_url: this.resEnvComponent.getUserKeyUrl()};

						}

				else if(!this.resEnvComponent.selectedTemplate.needs_forc_support){
												playbook_info[this.resEnvComponent.selectedTemplate.template_name] = {needs_forc_support: this.resEnvComponent.selectedTemplate.needs_forc_support}
