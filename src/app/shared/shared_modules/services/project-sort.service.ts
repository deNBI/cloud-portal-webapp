...
import {compare, SortColumn, SortDirection, State} from '../directives/nbd-sortable-header.directive';
...
export class ProjectSortService {
		...
		_initiateFilterStatusList(): void {
				...
				this.filterStatusList = [Application_States.ACTIVE,
		...
		matches(text: string, project: Application): boolean {
				...
				if (!this.showSimpleVM && !project.project_application_openstack_project || !this.showOpenStack && project.project_application_openstack_project) {
						return false
				...
				if (this.filterStatusList) {
						...
						let status_match: boolean = project.project_application_status.some(r => this.filterStatusList.includes(r))
						if (!status_match) {
								return false
						}
				}
				...
				return project.project_application_shortname.toLowerCase().includes(term)
		...
		sort(countries: Application[], column: SortColumn, direction: string): Application[] {
				if (direction === '' || column === '') {
						return countries;
				} else {
