...
export class VoService {
	...
	setClusterAllowed(groupid: number | string, set: boolean): Observable<any> {
		const parameters: HttpParams = new HttpParams().set('allowed', set ? 'allowed' : 'disallowed');

		return this.http.get(`${ApiSettings.getApiBaseURL()}vo/projects/${groupid}/toggle_cluster/`, {
			withCredentials: true,
			params: parameters,
		});
	}
