...
export class BiocondaService {
	...
	getAllTools(page: number, name?: string): Observable<any> {
		const params: HttpParams = new HttpParams().set('page', page.toString()).set('name', name)
	...
	getForcTemplates(clientid: string): Observable<ResearchEnvironment[]> {
		const params: HttpParams = new HttpParams().set('clientid', clientid)

		return this.http
			.get<ResearchEnvironment[]>(`${ApiSettings.getApiBaseURL()}forc/templates/`, {
				withCredentials: true,
				params
			})
			.pipe(
				map((resenvs: ResearchEnvironment[]): ResearchEnvironment[] =>
					resenvs.map((resenv: ResearchEnvironment): ResearchEnvironment => new ResearchEnvironment(resenv))
				)
			)
	}

	getSuggestedForcTemplates(facility_id?: string): Observable<any> {
		const params: HttpParams = new HttpParams().set('facility_id', facility_id)
